...
// There are more music-card elements in the HTML than entries in the
// songs array (extra placeholder cards), so only update as many cards
// as we actually have song data for.
allMusic.slice(0, songs.length).forEach((element, i) => {
  element.getElementsByTagName("img")[0].src = songs[i].songImage;
  element.getElementsByClassName("music-title")[0].innerText =
    songs[i].songName;
  element.getElementsByClassName("music-description")[0].innerText =
    songs[i].songDes;
});
...
function updateNowBar() {
  ...
  nowBar.getElementsByClassName("img-title")[0].innerText =
    order[currentSong - 1].songName;
  nowBar.getElementsByClassName("img-description")[0].innerText =
